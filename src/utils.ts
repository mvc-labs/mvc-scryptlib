...
export const DEFAULT_FLAGS =
  ...
  Interp.SCRIPT_ENABLE_MAGNETIC_OPCODES | Interp.SCRIPT_ENABLE_MONOLITH_OPCODES | // TODO: to be removed after upgrade to mvc 2.0
...
export function signTx(tx: mvc.Transaction, privateKey: mvc.PrivateKey, lockingScript: Script, inputAmount: number, inputIndex = 0, sighashType = DEFAULT_SIGHASH_TYPE, flags = DEFAULT_FLAGS): Sig {
  ...
  const buf = toHex(mvc.Transaction.sighash.sign(
    tx, privateKey, sighashType, inputIndex,
    lockingScript, new mvc.crypto.BN(inputAmount), flags
  ).toTxFormat());
  return new Sig(buf);
}
...
export function getPreimage(tx: mvc.Transaction, lockingScript: Script, inputAmount: number, inputIndex = 0, sighashType = DEFAULT_SIGHASH_TYPE, flags = DEFAULT_FLAGS): SigHashPreimage {
  const preimageBuf = mvc.Transaction.sighash.sighashPreimage(tx, sighashType, inputIndex, lockingScript, new mvc.crypto.BN(inputAmount), flags);
  return new SigHashPreimage(preimageBuf.toString('hex'));
}

const MSB_THRESHOLD = 0x7e;

export function getLowSPreimage(tx: mvc.Transaction, lockingScript: Script, inputAmount: number, inputIndex = 0, sighashType = DEFAULT_SIGHASH_TYPE, flags = DEFAULT_FLAGS): SigHashPreimage {
...
export function hash160(hexstr: string): string {
  return mvc.crypto.Hash.sha256ripemd160(Buffer.from(hexstr, 'hex')).toString('hex');
}

// Equivalent to the built-in function `sha256` in scrypt
export function sha256(hexstr: string): string {
  return mvc.crypto.Hash.sha256(Buffer.from(hexstr, 'hex')).toString('hex');
}

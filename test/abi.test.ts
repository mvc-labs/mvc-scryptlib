...
import { mvc, toHex, signTx } from '../src/utils'
...
describe('FunctionCall', () => {
  ...
  describe('when it is the contract constructor', () => {
    before(() => {
      target = new FunctionCall('constructor', {
        contract: p2pkh,
        lockingScript: p2pkh.lockingScript,
        args: [
          {
            name: 'pubKeyHash',
            type: 'Ripemd160',
  ...
  describe('when it is a contract public function', () => {
    ...
    before(() => {
      ...
      target = new FunctionCall('unlock', {
        contract: p2pkh,
        unlockingScript: mvc.Script.fromASM([sig.toASM(), pubkey.toASM()].join(' ')),
        args: [
          {
            name: 'sig',
            type: 'Sig',
    ...
    describe('toHex() / toString()', () => {
      it('should return the unlocking script in hex', () => {
        assert.equal(target.toHex(), mvc.Script.fromASM(target.toASM()).toHex())
      })
    })
  ...
  describe('when constructor with struct', () => {
    before(() => {
      target = new FunctionCall('constructor', {
        contract: person,
        lockingScript: person.lockingScript,
        args: [
          {
            name: 'some',
            type: 'Person',
  ...
  describe('struct type check', () => {
    it('should throw with wrong struct type', () => {
      ...
      }).to.throw('The type of p is wrong, expected Person but got Block')
    })

    it('should throw with wrong struct type', () => {
      expect(() => {
        new PersonContract(block, 18)
      }).to.throw('The type of some is wrong, expected Person but got Block')
    })
  })
})
  ...
  describe('test DemoCoinToss', () => {
    ...
    describe('test Demo', () => {
      ...
      it('should throw  with string not in hex or decimal', () => {
        expect(() => {
          let demo = new Demo(new Int('fasdfeeeeyjtuykjtukj'), 33)
        }).to.be.throw(
          "can't construct Int from <fasdfeeeeyjtuykjtukj>, Only supports integers, should use integer number, bigint, hex string or decimal string",
        )

        expect(() => {
          let demo = new Demo('fasdfeeeeyjtuykjtukj', 33)
        }).to.be.throw(
          "can't construct Int from <fasdfeeeeyjtuykjtukj>, Only supports integers, should use integer number, bigint, hex string or decimal string",
        )
        ...
        expect(() => {
          ...
          "can't construct Int from <afe3>, Only supports integers, should use integer number, bigint, hex string or decimal string",
        )

        expect(() => {
          new Int(1.3)
        }).to.be.throw(
          "can't construct Int from <1.3>, Only supports integers, should use integer number, bigint, hex string or decimal string",
        )

        expect(() => {
          new Int(1.401e30)
        }).to.be.throw(
          "can't construct Int from <1.401e+30>, <1.401e+30> is not safe integer, should use bigint, hex string or decimal string",
        )
        ...
        expect(() => {
          ...
          "can't construct Int from <-9007199254740992>, <-9007199254740992> is not safe integer, should use bigint, hex string or decimal string",
        )
        ...
        expect(() => {
          ...
          "can't construct Int from <1.401e-30>, Only supports integers, should use integer number, bigint, hex string or decimal string",
        )
      })
    })
    ...
    describe('test MDArray', () => {
      ...
      expect(() => {
        ...
        "can't construct Int from <1111111111111111h1111111111111111111>, Only supports integers, should use integer number, bigint, hex string or decimal string",
      )
    })
    ...
    describe('test encodePubFunctionCallFromHex', () => {
      it('test encodePubFunctionCallFromHex: PersonContract.equal', () => {
        ...
        const funCallClone = PersonContract.abiCoder.encodePubFunctionCallFromHex(
          mainClone,
          'equal',
          funCall.unlockingScript.toHex(),
        )
        ...
        const funCallCloneWrong = PersonContract.abiCoder.encodePubFunctionCallFromHex(
          mainClone,
          'equal',
          funCallWrongArgs.unlockingScript.toHex(),
        )
      ...
      it('test encodePubFunctionCallFromHex: DemoP2PKH.unlock', () => {
        ...
        const funCallClone = DemoP2PKH.abiCoder.encodePubFunctionCallFromHex(
          p2pkhClone,
          'unlock',
          funCall.unlockingScript.toHex(),
        )

...
describe('Test sCrypt contract stateProp In Javascript', () => {
  ...
  before(() => {
    ...
    test = new StateProp([new ST({ x: 0, y: true, z: new Bytes('00') }), new ST({ x: 0, y: true, z: new Bytes('00') })])

    let newLockingScript = test.getNewStateScript({
      st: [new ST({ x: 0, y: true, z: new Bytes('00') }), new ST({ x: 1, y: false, z: new Bytes('0001') })],
    })

    ...
    describe('Library with generic', () => {
      describe('LibGenericAsProperty1 test: can inferr all generic types from constructor', () => {
        ...
        it('should throw when using bool to new L', () => {
          expect(() => new Test(2, new L(true, true))).to.throw(
            'The type of l is wrong, expected L<int> but got L<bool>',
          )
        })

        it('should throw when using Bytes to new L', () => {
          expect(() => new Test(2, new L(new Bytes(''), new Bytes('')))).to.throw(
            'The type of l is wrong, expected L<int> but got L<bytes>',
          )
        })
      })
      ...
      describe('LibGenericAsProperty4 test: can inferr all generic types in nested library', () => {
        ...
        it('should succeeding', () => {
          instance = new TestGenericLibray(
            new GenericLibray(new GenericA(new ST({ a: 101, b: new Bytes('0f010f') })), 11),
          )
          result = instance.unlock(11).verify()
          expect(result.success, result.error).to.be.true
        })
        ...
        it('should throw when wrong constructor args ', () => {
          expect(() => new GenericLibray([new GenericA(new ST({ a: 101, b: new Bytes('0f010f') })), 11])).to.throw(
            `can't construct libraryClass from <[{101,b'0f010f'}],11>, The type of GenericLibray is wrong, expected [GenericA<ST>,T] but got [[[{"a":101,"b":"b'0f010f'"}],11]]`,
          )
        })
      })
    })

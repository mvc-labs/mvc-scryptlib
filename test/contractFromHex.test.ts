...
import { buildContractClass, buildTypeClasses, AbstractContract, VerifyResult } from '../src/contract'
import { mvc, signTx, toHex, toLiteral } from '../src/utils'
...
describe('create instance from UTXO Hex', () => {
  ...
  it('should throw when the raw script cannot match the ASM template of contract Simple', () => {
    ...
    expect(() => {
      ...
      Simple.fromHex(new mvc.Script.fromASM(asm).toHex())
    ...
    describe('.codePart', () => {
      it('should return the partial locking script (the part before op_return) of the contract', () => {
        ...
        assert.equal(
          ...
          `0014${toHex(pubKeyHash)}610079527a75517a75615179a95179876952795279ac7777776a`,
        )
      })
    })
    ...
    describe('.dataPart', () => {
      ...
      describe('when dataPart is to be set using setter', () => {
        it('should throw', () => {
          ...
          }).to.throw('Setter for dataPart is not available. Please use: setDataPart() instead')
        })
      })
    ...
    describe('constructorArgs', () => {
      describe('when build a contract which have Implicit constructor from asm', () => {
        ...
        it('should get right constructor args', () => {
          let args = [
            2,
            new Int(BigInt(11111111111111111111n)),
            false,
            new Bytes('1234567890'),
            new ST2({
              x: false,
          ...
          assert.deepEqual(
            toLiteral(newContract.ctorArgs().map((i) => i.value)),

...
import { mvc, toHex, signTx, num2bin, getPreimage, readLaunchJson } from '../src/utils'
...
describe('VerifyError', () => {
  ...
  describe('check VerifyError tokenUtxo.scrypt', () => {
    ...
    const privateKey1 = new mvc.PrivateKey.fromWIF('cMwKrDrzN5YPRHvPAAn9SfbQcXvARzpdtuufFQZZTBvBaqDETPhP')
    const publicKey1 = mvc.PublicKey.fromPrivateKey(privateKey1)
    const pkh1 = mvc.crypto.Hash.sha256ripemd160(publicKey1.toBuffer())
    const privateKey2 = new mvc.PrivateKey.fromRandom('testnet')
    const publicKey2 = mvc.PublicKey.fromPrivateKey(privateKey2)
    const privateKey3 = new mvc.PrivateKey.fromRandom('testnet')
    const publicKey3 = mvc.PublicKey.fromPrivateKey(privateKey3)
    ...
    const testSplit = (
      ...
      inputlockingScriptASM = undefined,
      inputAmount = 0,
    ) => {
      ...
      tx.addInput(
        ...
        mvc.Script.fromASM(token.lockingScript.toASM()),
        inputSatoshis,
      )

      const newLockingScript0 =
        [lockingScriptCodePart, toHex(publicKey2) + num2bin(0, DataLen) + num2bin(balance0, DataLen)].join(' ') + ' '
      tx.addOutput(
        new mvc.Transaction.Output({
          script: mvc.Script.fromASM(newLockingScript0),
      ...
      if (balance1 > 0) {
        ...
        tx.addOutput(
          new mvc.Transaction.Output({
            script: mvc.Script.fromASM(newLockingScript1),
            satoshis: outputAmount,
          }),
        )
      }
      ...
      const preimage = getPreimage(
        tx,
        inputlockingScriptASM ? mvc.Script.fromASM(inputlockingScriptASM) : token.lockingScript,
  ...
  describe('check VerifyError', () => {
    ...
    it('stop at p2pkh.scrypt#10', () => {
      let sig = signTx(tx, mvc.PrivateKey.fromRandom('testnet'), p2pkh.lockingScript, inputSatoshis)
  ...
  describe('check asmArgs', () => {
    ...
    it('it should succeeding when replace correctly ', () => {
      ...
      assert.deepEqual(
        ...
        '0 OP_PICK OP_NOP OP_5 ab12 OP_SIZE OP_NIP OP_MUL OP_1 OP_MUL OP_5 OP_SUB OP_EQUAL OP_NOP OP_NIP',
      )
  ...
  describe('check genLaunchJson opreturn', () => {
    ...
    it('it should contain opReturn', () => {
      ...
      tx.addOutput(
        new mvc.Transaction.Output({
          script: mvc.Script.fromASM(newLockingScript),
          satoshis: outputAmount,
        }),
      )

...
import { mvc, getPreimage, toHex, num2bin } from '../src/utils'
...
const privateKeys = [1, 1, 1, 1, 1].map((k) => mvc.PrivateKey.fromRandom())
const publicKeys = new Array(privateKeys.length)
const pkhs = new Array(privateKeys.length)

for (let k = 0; k < privateKeys.length; k++) {
  publicKeys[k] = mvc.PublicKey.fromPrivateKey(privateKeys[k])
  pkhs[k] = mvc.crypto.Hash.sha256ripemd160(publicKeys[k].toBuffer())
}
...
describe('Test advancedTokenSale.test', () => {
  ...
  function testBuy(numBought, pkh, publicKey) {
    ...
    tx.addOutput(
      new mvc.Transaction.Output({
        script: mvc.Script.buildPublicKeyHashOut(publicKey.toAddress()),
